...
function App() {
  ...
  const handleUpdateBook = useCallback(async (data: Partial<Book>) => {
    ...
    try {
      const updated = await window.api.books.update(currentBook.id, data);
  ...
  // 切换书籍
  const handleSwitchBook = useCallback(async (book: Book) => {
    await window.api.books.setCurrent(book.id);
    setCurrentBook(book);
    setCurrentPage('overview');
  }, []);
  ...
  const handleDeleteBook = useCallback(async (bookId: string) => {
    ...
    try {
      ...
      if (currentBook?.id === bookId) {
        ...
        if (remaining[0]) {
          await window.api.books.setCurrent(remaining[0].id);
        }
      }
    } catch (error) {
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-14 bg-slate-800 border-b border-slate-700 flex items-center justify-between px-6 drag-region">
          ...
          <div className="flex items-center gap-2 no-drag">
            ...
            <span className="text-sm text-gray-400">
              {currentBook ? `${currentBook.totalWordCount || 0}字` : ''}
            </span>
          </div>
        </header>
